...
module.exports = {
    ...
    getPublicPosts: function( connection, roomId, callback ){
        ...
        connection.query('SELECT temp.*, c.icon, c.chat_name_color, c.chat_text_color FROM (SELECT * FROM chat_log WHERE chat_room_id = 1 AND chat_log_type_id=? ORDER BY chat_log_id DESC LIMIT 50) AS temp JOIN `character` c ON temp.character_id=c.character_id ORDER BY chat_log_id ASC;', [roomId], function(err, rows, fields) {
            if (err){
                callback({
                    success: false,
                    err: err
